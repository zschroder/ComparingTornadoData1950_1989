...
SigTor.dat <- HistBigDays.sfdfT %>%
  group_by(Year) %>%
  summarize(Hist_bigdays = n(),
            Hist_numTorn = sum(nT), 
            Hist_avgnT = round(mean(nT)))

Year <- 1951
Hist_bigdays = 0
Hist_numTorn = 0
Hist_avgnT = 0
test <- cbind(Year, Hist_bigdays, Hist_numTorn, Hist_avgnT)
...
ONETOR.dat <- ModernBigDays.sfdfT %>%
  group_by(Year) %>%
  summarize(Mod_bigdays = n(),
            Mod_numTorn = sum(nT), 
            Mod_avgnT = round(mean(nT)))
...
dat <- merge(SigTor.dat, ONETOR.dat, by = "Year", all.x = FALSE)
```
...
Null Hypothesis: The means are the same between the groups. 

Alternative Hypothesis: There means are different between the groups.
```{r}
